...
sum<-newtable1%>%
  ...
  #filter(cand_name%in%c("PIERLUISI, PEDRO R","ALOMAR, RAFAEL COX","DELBENE, SUZAN K","PAYNE, DONALD M., JR.","MASSIE, THOMAS H.","BENTIVOLIO, KERRY","FATTAH, CHAKA MR.","MCDERMOTT, JAMES","KOSTER, JOHN","PETERS, GARY"))
sum
```


```{r}
graph_table<-sum%>%full_join(vote, by="cand_name")%>%
...
```{r}
ggplot(graph_table,aes(x=sum,y=general_votes))+geom_point()+geom_smooth()
```


```{r}
newGraph_table = graph_table%>%
  filter(general_votes<750000)%>%
  ggplot(aes(x = sum, y =general_votes))+
